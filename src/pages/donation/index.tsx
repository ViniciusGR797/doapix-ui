import styles from "./styles.module.scss";
import Head from "next/head";
import { Inter } from "next/font/google";
import HeaderLogo from "@/components/HeaderLogo";
import LoadingBar from "@/components/LoadingBar";
import { Button } from "@/components/Button";
import { useState } from "react";
import Router from 'next/router';
import Image from "next/image";
import { FiX } from "react-icons/fi";
import Link from "next/link";
import KeyValueDisplay from "@/components/KeyValueDisplay";
import ButtonCopy from "@/components/ButtonCopy";

const inter = Inter({ subsets: ["latin"] });

export default function Donation() {
    const [loadingCreatePayment, setLoadingCreatePayment] = useState(false);

    const handleButtonCreatePaymentClick = () => {
        setLoadingCreatePayment(true);
        Router.push('/create-payment')
        setLoadingCreatePayment(false);
    };

    const formatDate = (dateString: string): string => {
        const date = new Date(dateString);
        const day = date.getDate();
        const month = date.getMonth() + 1;
        const year = date.getFullYear();

        const formattedDay = day < 10 ? '0' + day : day;
        const formattedMonth = month < 10 ? '0' + month : month;

        const formattedDate = `${formattedDay}/${formattedMonth}/${year}`;

        return formattedDate;
    };

    const formatCurrency = (value: number): string => {
        const formattedValue = value.toLocaleString('pt-BR', {
            style: 'currency',
            currency: 'BRL'
        });

        return formattedValue;
    };

    const data = {
        "id": "f1cd0ab3-4f34-4a53-8b2c-594dd917d6ca",
        "url_image": "data:image/jpeg;base64,",
        "name": "World Wide Fund for Nature",
        "description": "Dia 24. No turbilhão da vida, às vezes nos deparamos com desafios que parecem insuperáveis. Uma doença súbita, um diagnóstico inesperado, uma cirurgia urgente. Nessas horas, a solidariedade se revela como um farol de esperança, guiando-nos através das trevas da incerteza. É nesse espírito que nasce esta vakinha, um elo de amor e apoio para alguém que enfrenta uma batalha pela saúde. Imagine-se de repente confrontado com a necessidade de uma cirurgia vital. Não apenas uma intervenção comum, mas um procedimento que pode significar a diferença entre a vida e a morte, entre o sofrimento contínuo e a chance de um futuro saudável. Essa é a realidade que muitos enfrentam diariamente, e é por isso que estamos aqui, unidos em um propósito comum: ajudar aqueles que precisam. A pessoa em questão é alguém como qualquer um de nós. Um pai, uma mãe, um irmão, uma irmã, um amigo querido. Alguém que sorri, que chora, que ama, que sonha. Mas agora, eles estão diante de um desafio monumental, e é nossa responsabilidade estender-lhes a mão. Não podemos deixar que o desespero e a angústia os consumam. Não podemos ficar de braços cruzados enquanto sua saúde pende em um fio. A cirurgia em questão é mais do que um procedimento médico. É a chave para desbloquear um futuro cheio de possibilidades. É a promessa de dias mais luminosos, de momentos preciosos compartilhados com aqueles que amamos. E é por isso que esta vakinha é tão crucial. Não se trata apenas de arrecadar fundos, mas de construir uma rede de apoio, de demonstrar que, quando nos unimos, somos capazes de realizar verdadeiros milagres. Cada doação, por menor que seja, é uma gota de esperança que se soma ao oceano de solidariedade. É um gesto de compaixão que ressoa através do tempo e do espaço, tocando corações e mudando vidas. E é importante lembrar que não se trata apenas do valor monetário, mas do espírito por trás dele. Cada contribuição é um testemunho do poder transformador do amor altruísta. Sabemos que o mundo pode parecer um lugar sombrio e implacável às vezes. Mas é em momentos como este que vemos o verdadeiro brilho da humanidade. Vemos pessoas de todas as esferas da vida se unindo em prol de um objetivo comum, transcendendo barreiras e diferenças para fazer a diferença na vida de alguém. E é uma coisa bela de se testemunhar. Portanto, convido você a se juntar a nós nesta jornada de esperança e solidariedade. Juntos, podemos fazer uma diferença real na vida de alguém que precisa desesperadamente de nossa ajuda. Juntos, podemos lançar uma luz brilhante no túnel escuro da doença. Juntos, podemos provar que, quando nos unimos em amor e compaixão, não há limites para o que podemos realizar. Então, estenda a mão. Doe o que puder. E juntos, vamos tornar possível o impossível.",
        "goal": 500.00,
        "amount_raised": 3.00, // Tratar no back, somar valor de transações pagas por doação
        "state": "AC",
        "created_at": "2024-03-24T19:34:02.090Z",
        "deadline": "2024-03-30T19:34:02.090Z",
        "category": "Projetos Sociais / Voluntariado",
        "transactions": 165, // Tratar no back, contar quantas transações pagas por doação
        "comments": [
            { "João": "🎁 Acabei de fazer uma doação para a sua causa incrível. Continue com o bom trabalho!" },
            { "Maria": "🤝 Fiz uma contribuição. Espero que ajude a fazer a diferença!" },
            { "Pedro": "💕 Doei para apoiar a sua missão. Vamos mudar o mundo juntos!" },
            { "": "🌱 Acabei de fazer uma doação. Que possamos crescer juntos nesta jornada!" },
            { "Jonas": "💪 Enviei uma doação para ajudar a alcançar nosso objetivo. Vamos lá!" },
            { "João": "🎁 Acabei de fazer uma doação para a sua causa incrível. Continue com o bom trabalho!" },
            { "Maria": "🤝 Fiz uma contribuição. Espero que ajude a fazer a diferença!" },
            { "Pedro": "💕 Doei para apoiar a sua missão. Vamos mudar o mundo juntos!" },
            { "": "🌱 Acabei de fazer uma doação. Que possamos crescer juntos nesta jornada!" },
            { "Jonas": "💪 Enviei uma doação para ajudar a alcançar nosso objetivo. Vamos lá!" },
        ],
    };

    return (
        <>
            <Head>
                <title>Doação</title>
                <meta name="viewport" content="width=device-width, initial-scale=1" />
                <link rel="icon" href="/favicon.ico" />
            </Head>

            <div>
                <div className={styles.header}>
                    <HeaderLogo />
                </div>
                <div className={styles.mainDiv}>
                    <div className={styles.donation}>
                        <div className={styles.body}>
                            <div className={styles.imageMetrics}>
                                <div className={styles.image}>
                                    <Image src={data.url_image} alt={data.name} width={300} height={300} style={{ objectFit: "cover" }} />
                                </div>
                                <div >

                                    <div className={styles.metricsDateNormal}>
                                        <KeyValueDisplay attribute={"Criada"} value={formatDate(data.created_at)} />
                                        <KeyValueDisplay attribute={"Finaliza"} value={formatDate(data.deadline)} />
                                    </div>
                                    <div className={styles.metricsDateMedia}>
                                        <KeyValueDisplay attribute={"Criada"} value={formatDate(data.created_at)} />
                                        <KeyValueDisplay attribute={"Finaliza"} value={formatDate(data.deadline)} />
                                        <KeyValueDisplay attribute={"Arrecadado"} value={formatCurrency(data.amount_raised)} />
                                        <KeyValueDisplay attribute={"Meta"} value={formatCurrency(data.goal)} />
                                        <KeyValueDisplay attribute={"Apoiadores"} value={data.transactions} />
                                    </div>

                                </div>
                            </div>
                            <div className={styles.info}>
                                <div className={styles.name}>
                                    <h2 className={styles.nameText} >{data.name}</h2>
                                    <Link href="/home" className={styles.link} >
                                        <FiX size={50} className={styles.exit} />
                                    </Link>
                                </div>
                                <div>

                                    <div className={styles.detailsNormal}>
                                        <div className={styles.description}>
                                            <p>{data.description}</p>
                                        </div>
                                        <div className={styles.comments}>
                                            {data.comments.map((comentario, index) => (
                                                <div key={index}>
                                                    {Object.entries(comentario).map(([nome, texto]) => (
                                                        <KeyValueDisplay attribute={nome ? nome : 'Anônimo'} value={texto} />
                                                    ))}
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    <div className={styles.detailsMedia}>
                                        <div className={styles.description}>
                                            <p>{data.description}</p>
                                        </div>
                                    </div>

                                </div>
                            </div>
                        </div>

                        <div className={styles.commentsMedia}>
                            <div className={styles.comments}>
                                {data.comments.map((comentario, index) => (
                                    <div key={index}>
                                        {Object.entries(comentario).map(([nome, texto]) => (
                                            <KeyValueDisplay attribute={nome ? nome : 'Anônimo'} value={texto} />
                                        ))}
                                    </div>
                                ))}
                            </div>
                        </div>

                        <div>
                            <div className={styles.metrics}>

                                <div className={styles.metricsNormal}>
                                    <KeyValueDisplay attribute={"Arrecadado"} value={formatCurrency(data.amount_raised)} />
                                    <KeyValueDisplay attribute={"Meta"} value={formatCurrency(data.goal)} />
                                    <KeyValueDisplay attribute={"Apoiadores"} value={data.transactions} />
                                    <ButtonCopy attribute={"Compartilhar doação"} value={data.id} />
                                </div>
                                <div className={styles.metricsMedia}>
                                    <ButtonCopy attribute={"Compartilhar doação"} value={data.id} />
                                </div>

                            </div>
                            <div>

                                <div className={styles.footerNormal}>
                                    <div className={styles.loadingBar}>
                                        <LoadingBar percentage={Math.round((data.amount_raised / data.goal) * 100)} />
                                    </div>
                                    <div className={styles.button}>
                                        <Button type="button" loading={loadingCreatePayment} style={{ width: '100%' }} onClick={handleButtonCreatePaymentClick} >Doar</Button>
                                    </div>
                                </div>
                                <div className={styles.footerMedia}>
                                    <div className={styles.button}>
                                        <Button type="button" loading={loadingCreatePayment} style={{ width: '100%' }} onClick={handleButtonCreatePaymentClick} >Doar</Button>
                                    </div>
                                </div>

                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </>
    )
}
